...
const elSelectSort = document.querySelector('.js-sort-select');
const elChooseSort = document.querySelector('.js-choose-select');

function render(array, elSearchInput) {
	elList.innerHTML = '';
	array.forEach(function(item){
		const liElement = document.createElement('li');
		liElement.setAttribute('class', 'item card list__item shadow')
		
		wrapperElement = document.createElement('ul');
		wrapperElement.setAttribute('class', 'wrapper card-body')
		
		const imgElement = document.createElement('img');
		imgElement.src = item.imageLink;
		
		const countryWrapperLiElement = document.createElement('li');
		const strongElementCountry = document.createElement('strong');
		strongElementCountry.classList.add('strong__add');
		strongElementCountry.textContent = 'Country : '
		const countryElement = document.createElement('p');
		countryElement.classList.add('country');
		countryElement.textContent = ` ${item.country}`;
		countryWrapperLiElement.append(strongElementCountry, countryElement)
		
		const authorWrapperLiElement = document.createElement('li');
		const authorElement = document.createElement('blockquote');
		authorElement.classList.add('author');
		authorElement.textContent = item.author;
		const strongElementAuthor = document.createElement('strong');
		strongElementAuthor.setAttribute('class','strong__add');
		strongElementAuthor.textContent = 'Author :'
		authorWrapperLiElement.append(strongElementAuthor, authorElement)
		...
		const pagesWrapperLiElement = document.createElement('li');
		const pagesElement = document.createElement('span');
		pagesElement.classList.add('pages');
		pagesElement.textContent = ` ${item.pages}`;
		const strongPagesElement = document.createElement('strong');
		strongPagesElement.textContent = 'Pages :'
		pagesWrapperLiElement.append(strongPagesElement,pagesElement);
		
		const titleElement = document.createElement('h2');
		titleElement.classList.add('title');
		titleElement.classList.add('card-title');
		
		const yearWrapperLiElement = document.createElement('li');
		const yearElement = document.createElement('span');
		yearElement.classList.add('year');
		yearElement.textContent = `${item.year}`;
		const strongYearElement = document.createElement('strong');
		...
		if(elSearchInput) {
			const regex = new RegExp(elSearchInput, 'gi');
			const marked = item.title.replace(regex, `<mark>${regex.source}</mark>`);
			titleElement.innerHTML = `${marked}`;
		}
		else {
...
elSelectSort.addEventListener('change', (evt) => {
	...
	if(evt.target.value == 'A') {
		const sortAZ = books.sort((a,b) => 
		a.title.toLowerCase().charCodeAt() - b.title.toLowerCase().charCodeAt(),  
		);
		render(books);
	}
	else {
		const sortedZA = books.sort((a,b) => 
		b.title.toLowerCase().charCodeAt() - a.title.toLowerCase().charCodeAt(),  
		);
		render(books);
	}
});
...
elSearchInput.addEventListener('keyup', (evt) => {
	...
	let finderSearch = books.filter(function(item) {
		let searchBooks = item.title.toLowerCase();
		return searchBooks.includes(searchValue);
	})
